...
### ADDED: Define a simple enemy policy function
def enemy_policy(enemy, allied_units):
    """
    Simple rule-based policy for enemy units:
      - With 50% probability, choose a random direction (or stay still).
      - Otherwise, move towards the closest allied unit.
    """
    import random
    if random.random() < 0.5:
        # Randomly select an action from up, right, down, left, or center
        return random.choice([ActionType.up, ActionType.right, ActionType.down, ActionType.left, ActionType.center])
    else:
        # Move toward the nearest allied unit (if any exist)
        if len(allied_units) == 0:
            return ActionType.center
        closest_unit = min(allied_units, key=lambda u: abs(u["x"] - enemy["x"]) + abs(u["y"] - enemy["y"]))
        dx = closest_unit["x"] - enemy["x"]
        dy = closest_unit["y"] - enemy["y"]
        if abs(dx) >= abs(dy):
            return ActionType.right if dx > 0 else ActionType.left
        else:
            return ActionType.down if dy > 0 else ActionType.up
...
class PPOGameEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        if self.current_step % 20 == 0:
            ...
            for enemy in self.enemy_units:
                ### ADDED: Call enemy_policy to determine enemy action
                chosen_action = enemy_policy(enemy, self.team_units)
                if chosen_action in [ActionType.up, ActionType.right, ActionType.down, ActionType.left]:
                    dx, dy = chosen_action.to_direction()
                else:
                    dx, dy = (0, 0)
                new_ex = enemy["x"] + dx
                new_ey = enemy["y"] + dy
                if 0 <= new_ex < SPACE_SIZE and 0 <= new_ey < SPACE_SIZE and self.tile_map[new_ey, new_ex] != 2:
